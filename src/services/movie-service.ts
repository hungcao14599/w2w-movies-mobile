...
class MovieService {
   ...
   */
  async getNewMovies(page: number = 1, limit: number = 20): Promise<MovieResponse> {
   ...
   */
  async getNewMoviesV2(page: number = 1, limit: number = 20): Promise<MovieResponse> {
   ...
   */
  async getNewMoviesV3(page: number = 1, limit: number = 20): Promise<MovieResponse> {
  ...
  async getMoviesByType(
    ...
    try {
      const result = await this.fetchFromAPI<any>(url, params);
      
      // API v1 returns: {status, msg, data: {items, params: {pagination}}}
      // Transform to: {status, items, pagination}
      if (result.data) {
        return {
      ...
      // Fallback if structure is already correct
      return result;
    } catch (error) {
